...
library(magrittr)
library(lmtest)
# install.packages(c('ri2','tidyr','RItools'))
# library(ri2)
library(data.table)
library(tidyr)
library(RItools)
...
lm1 = df[,lm(active_in_exp ~ treatment + sms)] # add sms
lm1$vcovCL1 = vcovCL(lm1, cluster = df[, c("address_group")])
#coeftest(lm1, vcov. = lm1$vcovCL1)

lm2 = df[,lm(active_in_exp ~ treatment + sms + phone)] # add phone
