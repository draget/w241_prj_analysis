...
```{r,message=FALSE,error=FALSE}
d = read.csv('~/Desktop/w241_prj_analysis/PrelimRESULTSTD.csv')
#g1 = read.csv('~/Desktop/group1.csv')
#g2_sms = read.csv('~/Desktop/group2_SMS.csv')
#g2_no_sms = read.csv('~/Desktop/group2_NoSMS.csv')
s_prior = read.csv('~/Desktop/w241_prj_analysis/scripts_priorTD.csv')
s_during = read.csv('~/Desktop/w241_prj_analysis/scripts_duringTD.csv')
```
...
qplot(distribution.under.sharp.null) +
 geom_vline(aes(xintercept = ATE,color = 'ATE')) + 
  ...
  ylab('Sampling Simulation') + 
  xlab('Distribution Under the Sharp Null Hypothesis')
```
...
lm3 = df[phone == 0,lm(active_in_exp ~ treatment + active_3m + treatment * active_3m)] # add active history
lm3$vcovCL1 = vcovCL(lm3, cluster = df[phone == 0, c("address_group")])
#coeftest(lm3, vcov. = lm3$vcovCL1)

lm4 = df[phone == 1, lm(active_in_exp ~ treatment)] # base model
...
lm7 = df[phone == 1, lm(active_in_exp ~ treatment + sms + treatment * sms + active_3m + treatment * active_3m)] # add sms
...
stargazer(lm1, lm2, lm3, lm4, lm5, lm6, lm7,
         ...
         type = 'text',header=FALSE,
         omit.stat = c('F', 'ser'),
         column.labels = c('No Phone', 'No Phone', 'No Phone', 'Has Phone', 'Has Phone', 'Has Phone', 'Has Phone'))
```
...
### Isolate people with phone

```{r}
lm2 = df[phone == 1, lm(active_in_exp ~ treatment)]
lm2$vcovCL1 = vcovCL(lm2, cluster = df[phone == 1, c("address_group")])
#coeftest(lm2, vcov. = lm2$vcovCL1)

lm1 = df[phone == 1, lm(active_in_exp ~ treatment + sms + treatment * sms)] 
lm1$vcovCL1 = vcovCL(lm1, cluster = df[phone == 1, c("address_group")])
#coeftest(lm1, vcov. = lm1$vcovCL1)

lm3 = df[phone == 1, lm(active_in_exp ~ treatment + sms + treatment * sms + active_3m + treatment * active_3m)]
lm3$vcovCL1 = vcovCL(lm3, cluster = df[phone == 1, c("address_group")])
#coeftest(lm3, vcov. = lm3$vcovCL1)


stargazer(lm2, lm1, lm3, 
          type = 'text', header = FALSE,
